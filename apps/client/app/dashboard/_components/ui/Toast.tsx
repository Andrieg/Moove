...
function ToastItem({ toast, onDismiss }: { toast: ToastMessage; onDismiss: (id: string) => void }) {
  ...
  const bgColors = { success: "bg-green-500", error: "bg-red-500", info: "bg-[#308FAB]" };
  const icons = {
  ...
  return (
    <div className={`${bgColors[toast.type]} text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 animate-slideUp`}>
      {icons[toast.type]}
      <span className="font-medium text-sm">{toast.message}</span>
      <button onClick={() => onDismiss(toast.id)} className="ml-2 hover:opacity-70 transition">
