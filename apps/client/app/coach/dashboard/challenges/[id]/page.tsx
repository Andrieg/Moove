...
export default function EditChallengePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    getChallengeById(id).then((c) => {
      if (c) setFormData({
        title: c.title || "",
        description: (c as any).description || "",
        startDate: c.startDate ? new Date(c.startDate).toISOString().split("T")[0] : "",
        endDate: c.endDate ? new Date(c.endDate).toISOString().split("T")[0] : "",
        type: (c as any).type || "daily",
        visibility: (c as any).visibility || "public",
      });
    }).catch(() => toast.error("Failed to load challenge")).finally(() => setIsLoading(false));
  }, [id]);
  ...
  return (
    <div className="max-w-3xl">
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Delete Challenge" size="sm">
